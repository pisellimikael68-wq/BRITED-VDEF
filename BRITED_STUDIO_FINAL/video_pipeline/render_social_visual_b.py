...
# Direction B validée : ivoire, encre profonde et modules architecturés.
...
def base(spec: dict, page: int):
    ...
    concept=(spec.get("concept_id") or "").casefold()
    topic=("RÉGIME MATRIMONIAL" if concept.startswith("regimes_") else
           {"fiscalite":"FISCALITÉ","immobilier":"IMMOBILIER","juridique":"JURIDIQUE","finance":"FINANCE"}.get(spec.get("serie"),"PATRIMOINE"))
    draw.text((48,43),topic,anchor="la",font=font(20),fill=TERRA)
    draw.rounded_rectangle((48,77,112,80),radius=2,fill=accent)
    total=len(spec["beats"])
    gap=9; left=48; right=672; segment=(right-left-gap*(total-1))/total
    for index in range(total):
        x1=left+index*(segment+gap); x2=x1+segment
        draw.rounded_rectangle((x1,1227,x2,1234),radius=4,fill=accent if index<=page else (230,229,225))
    return image,draw


def question_page(spec: dict,page: int,beat: dict):
    image,draw=base(spec,page); accent=series_color(spec)
    draw.text((360,425),"À VOUS DE JOUER",anchor="mm",font=font(17),fill=TERRA)
    question=re.sub(r"(?:[,;:]?\s*(?:OUI\s*(?:/|OU)\s*NON)\s*[.!]?)$","",beat["narration"].upper()).strip()
    words=question.split()
    face=display_font(40); lines=wrap_words(draw,words,face,624)
    ...
    y=555-(len(lines)-1)*gap/2
    for line in lines:
        draw.text((360,y)," ".join(line),anchor="mm",font=face,fill=INK); y+=gap
    draw.rounded_rectangle((190,690,350,754),radius=11,fill=accent)
    draw.text((270,722),"OUI",anchor="mm",font=display_font(23),fill=WHITE)
    draw.rounded_rectangle((370,690,530,754),radius=11,fill=WHITE,outline=INK,width=2)
    draw.text((450,722),"NON",anchor="mm",font=display_font(23),fill=INK)
    draw.line((48,1027,672,1027),fill=(228,225,217),width=1)
    return image
...
def answer_page(spec: dict,page: int,beat: dict,visible: int | None = None):
    ...
    first=answer_words[0].rstrip(".:;!?") if answer_words else "NON"
    remainder=answer_words[1:]
    face=display_font(40); lines=wrap_words(draw,remainder,face,624)
    while len(lines)>4 and face.size>36:
        face=display_font(face.size-2); lines=wrap_words(draw,remainder,face,545)
    pill_width=max(112,round(draw.textlength(first,font=display_font(21))+74))
    draw.rounded_rectangle((360-pill_width/2,430,360+pill_width/2,486),radius=11,fill=INK)
    draw.text((360,458),first,anchor="mm",font=display_font(21),fill=WHITE)
    gap=round(face.size*1.18); y=585-(len(lines)-1)*gap/2
    shown=0
    for line in lines:
...
def cta_page(spec: dict,page: int,beat: dict,progress: float = 1.0):
    ...
    if True:
        ...
        face=display_font(48); lines=wrap_words(draw,question,face,624); y=390-(len(lines)-1)*28
        for line in lines:
            draw.text((360,y)," ".join(line),anchor="mm",font=face,fill=fade_color(WHITE,INK,question_alpha)); y+=57
        button_fill=fade_color(WHITE,SOFT,button_alpha)
        button_stroke=fade_color(WHITE,accent,button_alpha)
        draw.rounded_rectangle((112,570,608,645),radius=13,fill=button_fill,outline=button_stroke,width=3)
        draw.text((360,608),"PRENEZ RENDEZ-VOUS",anchor="mm",font=display_font(25),fill=button_stroke)
        destination="VIA LE LIEN DANS MA BIO"
        destination_color=fade_color(WHITE,TERRA,destination_alpha)
        draw.text((360,690),destination,anchor="mm",font=font(19),fill=destination_color)
        draw.line((48,1027,672,1027),fill=(228,225,217),width=1)
        # Le calendrier de la spécification est volontairement sans carré.
        if LUCIDE_FONT.exists() and "calendar" in _LUCIDE_POINTS:
            glyph=chr(int(_LUCIDE_POINTS["calendar"]))
            draw.text((360,1136),glyph,anchor="mm",font=ImageFont.truetype(str(LUCIDE_FONT),48),fill=destination_color)
        return image
    ...
    boxes=[(91,520,251,655),(280,520,440,655),(469,520,629,655)]
    for i,box in enumerate(boxes): draw.rounded_rectangle(box,radius=10,fill=SOFT if i!=1 else WHITE,outline=accent if i!=1 else TERRA,width=3)
...
def library_icon(draw,kind,accent) -> bool:
    ...
    count=len(names); card=112; gap=19
    total=count*card+(count-1)*gap; left=(W-total)//2; y=1080
    for index,name in enumerate(names):
        x=left+index*(card+gap); active=index==min(1,count-1)
        fill=SOFT if active else WHITE; stroke=TERRA if active else accent
        draw.rounded_rectangle((x,y,x+card,y+card),radius=13,fill=fill,outline=stroke,width=2)
        glyph=chr(int(_LUCIDE_POINTS[name])); glyph_font=ImageFont.truetype(str(LUCIDE_FONT),48)
        draw.text((x+card/2,y+card/2),glyph,anchor="mm",font=glyph_font,fill=stroke)
        if index<count-1:
            cx=x+card+gap/2; draw.line((cx-5,y+card/2,cx+5,y+card/2),fill=SAND,width=2)
    return True
...
TEXT_SAFE_TOP, TEXT_SAFE_BOTTOM = 113, 1027
ICON_SAFE_TOP, ICON_SAFE_BOTTOM = 1080, 1192
...
def regular_layout(beat: dict):
    ...
    face=display_font(56); lines=wrap_words(draw,words,face,624)
    if len(lines)>3:
        face=display_font(48); lines=wrap_words(draw,words,face,624)
    if len(lines)>4:
        face=display_font(40); lines=wrap_words(draw,words,face,624)
    gap=round(face.size*1.18); text_height=(len(lines)-1)*gap+face.size
    while (len(lines)>6 or text_height>TEXT_SAFE_BOTTOM-TEXT_SAFE_TOP) and face.size>30:
...
def regular_page(spec: dict,page: int,beat: dict,visible: int,icon_alpha: float = 1.0):
    ...
    exact_icons={
        "hook":"users heart-handshake",
        "regle":"users calendar",
        "precision":"scale shield-check users",
        "exemple":"landmark shield-check",
        "nuance":"file-text users badge-percent",
    }
    icon(layer_draw,exact_icons.get(beat.get("id"),visual_context(beat)),accent)
...
def render(spec_path: Path, output: Path, timing_path: Path) -> None:
    ...
    trace={"renderer":"BRITED deterministic renderer v2","features":{
        ...
        "visual_direction": "Claude Design - Visuel B",
    ...
    for beat,duration,speech_duration in zip(spec["beats"],durations,speech_durations):
        ...
        if role not in {"question","reponse","answer","cta","cloture"}:
            ...
            exact_icons={"hook":"users heart-handshake","regle":"users calendar",
                         "precision":"scale shield-check users","exemple":"landmark shield-check",
                         "nuance":"file-text users badge-percent"}
            visual_layout={"text_bounds":[48,top,W-48,top+text_height],
                           "icon_bounds":[48,ICON_SAFE_TOP,W-48,ICON_SAFE_BOTTOM],
                           "selected_icons":visual_icon_names(exact_icons.get(beat.get("id"),visual_context(beat))),
                           "font_size":face.size,
