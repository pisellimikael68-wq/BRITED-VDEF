    ...
    def prepare(self, start: date, days: int, platforms: list[str]) -> dict[str, Any]:
        with self.lock:
            ...
            for publication in selected:
                ...
                certified = bool(relative and validation.get("validated") and float(validation.get("score", 0)) == 10.0 and float(validation.get("threshold", 0)) == 10.0)
                if certified:
                    candidate = self.store.root / relative
                    if candidate.is_file():
                        source = candidate.read_text(encoding="utf-8")
                        audit = audit_script(source, publication["platform"])
                        # A legacy voice-over/slide script may remain certified
                        # historically, but it is not the new face-camera master.
                        certified = bool(audit.ok and not audit.warnings
                                         and "## Guide de tournage" in source
                                         and "## Incrustations" in source)
                    else:
                        certified = False
            ...
            policy_path = self.store.data / "editorial_policy.json"
            try:
                policy = json.loads(policy_path.read_text(encoding="utf-8")) if policy_path.exists() else {}
            except (OSError, json.JSONDecodeError):
                policy = {}
            remediation_rounds = max(1, min(5, int(policy.get("automation", {}).get("maximum_remediation_cycles", 3))))
            state = {"status": "running", "started_at": datetime.now().isoformat(timespec="seconds"),
                     ...
                     "remediation_round": 0, "maximum_remediation_rounds": remediation_rounds, "items": items}
    ...
    def _run(self) -> None:
        while True:
                ...
                if item is None:
                    ...
                    if blocked and remediation_round < maximum_rounds:
                        for blocked_item in blocked:
                            blocked_item.update(status="pending")
                            blocked_item.setdefault("failure_history", []).append({
                                "round": remediation_round,
                                "error": blocked_item.get("error", ""),
                                "path": blocked_item.get("path", ""),
                                "review_path": blocked_item.get("review_path", ""),
                            })
            ...
            try:
                ...
                error = "" if status == "certified" else " · ".join(findings[:3]) or "Certification strictement inférieure à 10/10"
