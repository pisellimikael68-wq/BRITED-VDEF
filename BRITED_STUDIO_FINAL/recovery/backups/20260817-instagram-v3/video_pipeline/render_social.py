...
def answer_page(spec: dict,page: int,beat: dict,visible: int | None = None):
    ...
    if visible is not None:
        answer_words=answer_words[:visible]
    ...
    face=font(54); lines=wrap_words(draw,answer_words,face,455)
...
def icon(draw,kind,accent):
    if any(x in kind for x in ("compte bancaire","salaire","épargne")):
    ...
    elif "3 masses" in kind or "trois masses" in kind:
    ...
    elif any(x in kind for x in ("documents","liquidation","partage")):
    ...
    elif any(x in kind for x in ("mariage","couple","époux")):
...
def render(spec_path: Path, output: Path, timing_path: Path) -> None:
        ...
        for page,(beat,duration) in enumerate(zip(spec["beats"],durations)):
            ...
            for n in range(frames):
                ...
                if kind=="question": image=question_page(spec,page,beat)
                    ...
                    visible,amount=reveal(answer_words,t,duration)
    ...
    trace={"renderer":"BRITED deterministic renderer v2","features":{
        "page_crossfade_seconds": .24,
        "progressive_word_reveal": True,
        "contextual_illustrations": True,
