...
from render_whiteboard import ffmpeg_executable
...
MUTED = (107, 91, 78)
SERIES = {"fiscalite": (149,83,0), "immobilier": (73,107,89),
          "juridique": (168,56,88), "finance": (49,87,164)}
...
def answer_page(spec: dict,page: int,beat: dict):
    ...
    face=font(84)
    while draw.textlength(answer,font=face)>460 and face.size>48: face=font(face.size-2)
    draw.text((360,505),answer,anchor="mm",font=face,fill=accent)
    return image
...
def cta_page(spec: dict,page: int,beat: dict):
    ...
    draw.text((171,671),"PARTAGEZ",anchor="mm",font=font(21),fill=accent)
    draw.rounded_rectangle((337,579,383,638),radius=7,outline=TERRA,width=4);draw.polygon(((341,630),(360,617),(379,630)),fill=TERRA)
    draw.text((360,671),"ENREGISTREZ",anchor="mm",font=font(18),fill=TERRA)
...
def regular_page(spec: dict,page: int,beat: dict,visible: int):
    image,draw=base(spec,page); accent=series_color(spec)
    words=beat["narration"].upper().split(); size=38 if len(words)>25 else 43; face=font(size)
